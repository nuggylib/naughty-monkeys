...
public class TemptOnlyByBananaHatWearersGoal<T extends LivingEntity> extends Goal {
    ...
    public TemptOnlyByBananaHatWearersGoal(PathfinderMob p_25939_, double p_25940_, Ingredient p_25941_, boolean p_25942_) {
        this.mob = p_25939_;
    ...
    public void tick() {
        ...
        List<ItemStack> armorSlots = (List<ItemStack>) this.player.getArmorSlots();
        boolean targetHasBananaHat = armorSlots.stream().filter(itemStack -> itemStack.getItem() instanceof BananaHat).toList().size() > 0;
        if (this.mob.distanceToSqr(this.player) < 6.25D || !targetHasBananaHat) {
            this.mob.getNavigation().stop();
        } else {
